    ...
    Users.add(user)
        ...
        .catch(err => {
            res
                .status(500) //error
                .json({
                    errMessage: `createUser error: ${err}`
                });
        });
};
        ...
        req.session.destroy(err => {
            if (err) {
                res
                    .send(401) //error
                    .json({
                        errMessage: "Logout errors for days!"
                    });
            } else {
                res
                    .status(202) //success (?)
    ...
    Users.remove(id)
        .then(deletedUser => {
            ...
            res
                .status(200) //success
                .json({
                    successfulMessage: `You have deleted ${id}`
                });
        })
        .catch(err => {
            res
                .status(500) //
                .json({
                    deleteErr: "could not delete user",
                    error: err
                });
        });
};
...
exports.updateUser = async (req, res, next) => {
    ...
    try {
        ...
        res
            .status(500) //fail
            .json({
                updateErr: "could not update user info"
            });
    }
};
